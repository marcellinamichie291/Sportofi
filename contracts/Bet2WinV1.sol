...
import "./interfaces/IBet2WinV1.sol";
...
contract Bet2WinV1Upgradeable is
    BaseUpgradeable,
    SignableUpgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function placeBet(
        ...
        ///@dev get rid of stack too deep
        {
            ...
            require(
                _hasRole(
                    Roles.SIGNER_ROLE,
                    _recoverSigner(structHash, croupierSignature_)
                ),
        ...
        emit BetPlaced(user, id, side, settleStatus, amount, odd);
    }
    ...
    function settleBet(
        ...
        _safeTransfer(
            _rewardToken,
            user,
            amount.mulDivDown(leverage, PERCENTAGE_FRACTION)
        );
        if (referree != address(0))
            _safeTransfer(
                _rewardToken,
                referree,
                amount.mulDivDown(REFERAL_PERCENT, PERCENTAGE_FRACTION)
            );
    }
